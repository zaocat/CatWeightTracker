/**
 * Cloudflare Worker - Cat Weight Tracker (v6.2 - Theme Toggle)
 * Features: Dark/Light Toggle, Time Filters, PWA, 3D Tilt, Fix Unicode
 */
...
export default {
  async fetch(request, env) {
    ...
    if (protectedPaths.includes(path)) {
      ...
      const validToken = btoa(unescape(encodeURIComponent(`${adminUser}:${adminPass}`)));
...
<style>
:root { --bg: linear-gradient(135deg,#fff1eb 0%,#ace0f9 100%); --card: rgba(255,255,255,0.85); --text: #444; --input: #f0f4f8; }
/* 登录页简单适配深色，跟随系统 */
@media (prefers-color-scheme: dark) { :root { --bg: linear-gradient(135deg,#1e293b 0%,#0f172a 100%); --card: rgba(30,41,59,0.85); --text: #f1f5f9; --input: #334155; } }
body{display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;background:var(--bg);font-family:'Varela Round',sans-serif;color:var(--text)}
.card{background:var(--card);backdrop-filter:blur(12px);padding:2.5rem;border-radius:24px;box-shadow:0 10px 30px rgba(0,0,0,0.1);width:100%;max-width:380px;border:1px solid rgba(255,255,255,0.2);text-align:center;transition:transform 0.3s}
.card:hover{transform:translateY(-5px)}
h1{margin-bottom:1.5rem;font-size:1.6rem}
input{width:100%;padding:14px;margin-bottom:1rem;border:none;background:var(--input);border-radius:12px;font-size:1rem;box-sizing:border-box;outline:none;transition:0.2s;font-family:inherit;color:var(--text)}
input:focus{box-shadow:0 0 0 2px #ff9a9e}
button{width:100%;padding:14px;background:linear-gradient(to right,#ff9a9e,#fecfef);color:#fff;border:none;border-radius:12px;font-size:1.1rem;cursor:pointer;font-family:inherit;box-shadow:0 4px 10px rgba(255,154,158,0.3);transition:0.2s}
button:hover{transform:translateY(-2px)}.err{color:#ff6b6b;margin-bottom:1rem;background:rgba(255,0,0,0.1);padding:8px;border-radius:8px;font-size:0.9rem}.back{display:block;margin-top:1.5rem;color:#888;text-decoration:none;font-size:0.9rem}
</style></head><body><div class="card"><h1>🐱 铲屎官登录</h1>${error?`<div class="err">${error}</div>`:''}<form action="/auth/login" method="POST"><input type="text" name="username" placeholder="用户名" required><input type="password" name="password" placeholder="密码" required><button type="submit">芝麻开门</button></form><a href="/" class="back">← 返回首页</a></div></body></html>`;
}
  ...
  const css = `
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    body {
      ...
      min-height: 100vh;
      perspective: 1000px;
      transition: color 0.3s, background 0.3s; /* 平滑过渡 */
    }
    ...
    .cat-switcher {
      ...
      box-shadow: 0 4px 20px var(--shadow); margin-bottom: 30px;
    ...
    .curr-cat { 
      ...
      font-weight: 800; font-size: 1.4rem; cursor: pointer; color: var(--text); 
    }
    ...
    .card {
      ...
      transition: transform 0.1s ease, box-shadow 0.2s ease, background 0.3s;
      backdrop-filter: blur(10px);
    }
    .card::after {
      content: ""; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
      background: radial-gradient(800px circle at var(--mouse-x) var(--mouse-y), rgba(255, 255, 255, 0.15), transparent 40%);
      opacity: 0; transition: opacity 0.5s; pointer-events: none; z-index: 1; mix-blend-mode: overlay;
    }
    ...
    .filter-btn {
      ...
      padding: 4px 12px; border-radius: 12px; cursor: pointer; transition: 0.2s; font-family: inherit;
    }
    ...
    .chart-tooltip {
      ...
      padding: 8px 14px; border-radius: 12px; font-size: 0.9rem; pointer-events: none; opacity: 0;
      transition: opacity 0.2s, top 0.2s, left 0.2s; transform: translate(-50%, -115%);
      white-space: nowrap; z-index: 10; box-shadow: 0 5px 15px rgba(0,0,0,0.3); text-align: center;
    }
    .chart-tooltip::after {
    ...
    .trend-up { color: #ff6b6b; background: rgba(255, 107, 107, 0.1); padding: 4px 12px; border-radius: 20px; font-size: 0.9rem; }
    .trend-down { color: #20bf6b; background: rgba(32, 191, 107, 0.1); padding: 4px 12px; border-radius: 20px; font-size: 0.9rem; }
    ...
    <script>
      ...
      function renderChart(data) {
        ...
        if(trend && data.length > 1) {
           ...
           const sign = diff > 0 ? '↗ +' : '↘ ';
           trend.innerHTML = \`<span class="\${cls}">\${sign}\${Math.abs(diff)}kg</span>\`;
